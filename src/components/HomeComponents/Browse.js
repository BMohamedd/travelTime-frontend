...
import {
  requestBrowsePostes,
  requestMoreBrowsePostes,
} from "../../redux/actionCreators/browseActionCreator";
    ...
    <div className="container">
      <div className="row d-flex flex-column align-items-center">
        <div className="col-12 col-md-10 col-lg-8 mt-5 d-flex flex-column justify-content-center align-items-center">
          <div
            className="mb-5 shadow-lg col-12 text-center py-4"
            onClick={() => navigate("saved-postes")}
            style={{ cursor: "pointer" }}
          >
            <h3>View Saved Postes!</h3>
          </div>
          <CodeAndBugReport />
          {postes.map((element) => {
            return (
              <Poste
                likeCounter={element.posts.likeCounter}
                picture={element.posts.picture}
                user={element.username}
                key={element.posts._id}
                id={element.posts._id}
                userId={element._id}
                date={element.posts.createdAT}
        ...
        <button
          className="mb-5 btn btn-outline-primary w-25"
          onClick={handleMore}
        >
          {" "}
          View more{" "}
